$(document).ready(function() {
	...
	$('#purposeTip').popover({
        html : true,
		placement: 'top',
        content: '<div class="popover-tip-text">Активным рефералом, считается тот кто пополнил баланс от 100р</div>'
    });
			...
			success: function(data) {
				...
				if(data.success) {
					...
					if(data.bonusType == 'refs') {
						$('.form_wrapper.refs .form_block').html('<div class="form_recharge">Вы получили данный бонус!</div>');
					}
				}
				$.notify({
					type: data.type,
	...
	function timer(remaining) {
		if(!remaining) {
			...
			$('.form_wrapper.group .form_block').html('<div class="form_recharge"><span>Перезарядка через:</span><div class="form_timeLeft">00:00:00</div></div>');
			return;
		}
		clearInterval(countdown);
		countdown = setInterval(() => {
			if(remaining == 0) {
				remaining == null;
				clearInterval(countdown);
				$('.form_wrapper.group .form_block').html('<div class="form_value">'+ settings.bonus_group_time +' мин<div class="form_text">перезарядка</div></div><span id="spin-wheel-button" class=""><button type="button" class="btn" data-toggle="modal" data-target="#captchaModal">Крутить колесо</button></span>');
				return;
			}
			remaining--;
			var coul = convertTimestamp(remaining);
			$('.form_wrapper.group .form_block').html('<div class="form_recharge"><span>Перезарядка через:</span><div class="form_timeLeft">'+ coul +'</div></div>');
		}, 1*1000);
	}
