$(document).ready(function() {
    ...
    socket.on('jackpot', r => {
        ...
        if(r.type == 'slider' && room == r.room) {
			...
			setTimeout(function () {
				var wbl = '';
				if(r.data.winner_balance > 0) wbl += '<div class="payout">'+ r.data.winner_balance +' <svg class="icon icon-coin balance balance"><use xlink:href="/img/symbols.svg#icon-coin"></use></svg></div>';
				var wbn = '';
				if(r.data.winner_bonus > 0) wbn += '<div class="payout">'+ r.data.winner_bonus +' <svg class="icon icon-coin balance bonus"><use xlink:href="/img/symbols.svg#icon-coin"></use></svg></div>';

				var html = '';
					html += '<div class="game-tooltip isTransparent won '+ r.room +'"><div class="wrap"><div class="user"><button type="button" class="btn btn-link" data-id="'+ r.data.winner_id +'"><span class="sanitize-user"><div class="sanitize-avatar"><img src="'+ r.data.winner_avatar +'" alt=""></div><span class="sanitize-name">'+ r.data.winner_name +'</span></span></button></div>'+ wbl + wbn +'<div class="badge"><div class="text">Vencedor</div></div><div class="status"><span>Tickets <span class="profit">'+ r.data.ticket +'</span> <svg class="icon"><use xlink:href="/img/symbols.svg#icon-ticket"></use></svg></span></div></div></div>';
        ...
        if(r.type == 'newGame') {
			if(room == r.room) {
				...
				chart.data.datasets[0].backgroundColor = ['#145250'];
				chart.update();
			}
    ...
    function JackpotParse(res) {
        ...
        res.bets.forEach(bet => {
            bets += '<tr><td class="username"> <button type="button" class="btn btn-link" data-id="'+bet.user.id+'"> <span class="sanitize-user"><div class="sanitize-avatar"><img src="'+bet.user.avatar+'" alt="" style="border: 1px solid #'+bet.bet.color+';"></div> <span class="sanitize-name">'+bet.user.username+'</span> </span> </button></td><td><div class="bet-number"> <span class="bet-wrap"> <span>'+bet.bet.amount+'</span> <svg class="icon icon-coin balance '+bet.bet.balance+'"> <use xlink:href="/img/symbols.svg#icon-coin"></use> </svg> </span></div></td><td>'+bet.bet.chance+'%</td><td><div class="bet-number rtl"> <span class="bet-wrap"> <span>'+bet.bet.from+' - '+bet.bet.to+'</span> <svg class="icon"> <use xlink:href="/img/symbols.svg#icon-ticket"></use> </svg> </span></div></td></tr>';
        });
        $('#bets').html(bets);
