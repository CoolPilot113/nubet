$(document).ready(function() {
    ...
    this.publishTime = (res) => {
        $('#chartInfo').text('Começando em... ' + res.value + 's.');
    }
    ...
    this.publishBet = (res) => {
        ...
        {
            let bet = res.bets[i];
            html += '<tr><td class="username"><button type="button" class="btn btn-link" data-id="'+ bet.user.unique_id +'"><span class="sanitize-user"><div class="sanitize-avatar"><img src="'+ bet.user.avatar +'" alt=""></div><span class="sanitize-name">'+ bet.user.username +'</span></span></button></td><td><div class="bet-number"><span class="bet-wrap"><span>'+ bet.price +'</span><svg class="icon icon-coin balance '+ bet.balType +'"><use xlink:href="/img/symbols.svg#icon-coin"></use></svg></span></div></td><td>х'+ bet.withdraw +'</td><td>';
            if(bet.status == 1) html += '<span class="bet-wrap win"><span>'+ bet.won +'</span><svg class="icon icon-coin balance"><use xlink:href="/img/symbols.svg#icon-coin"></use></svg></span>';
    ...
    this.reset = (res) => {
        $('#bets').html('');
        $('#chartInfo').css('color', '#ffc645').text('Conectando...');
    ...
    this.parseSlider = (res) => {
        ...
        $('#chartInfo').text(((res.crashed) ? 'Crashou ' : '') + 'x' + res.float.toFixed(2));
        if(res.crashed)
        {
            ...
            if(!window.isCashout && window.withdraw > 0) $('.btn-withdraw span').text('Retirar ' + parseFloat(window.bet*parseFloat(res.float.toFixed(2))).toFixed(2));
