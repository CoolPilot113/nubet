...
$(document).ready(function() {
            ...
            success : function(data) {
				if(data.success) {
					...
					for(var i = 0; i < data.users.length; i++) {
						...
						html += '<td>'+ (data.users[i].banchat_reason ? data.users[i].banchat_reason : 'Не указана') +'</td>';
						html += '<td>';
						html += '<button type="button" class="btn btn-light btnUnBan" data-name="' + data.users[i].username + '" data-id="' + data.users[i].unique_id + '"><svg class="icon"><use xlink:href="/img/symbols.svg#icon-ban"></use></svg><span>Разбанить</span></button>';
						html += '</td>';
						html += '</tr>';
					}
	...
	.on('ban_message', function(data) {
		if(data.ban) {
			...
			$('.message-block.user_' + data.unique_id + ' .delete').append('<button type="button" class="btn btn-light btnUnBan" data-name="' + data.username + '" data-id="' + data.unique_id + '"><svg class="icon"><use xlink:href="/img/symbols.svg#icon-ban"></use></svg><span>Разбанить</span></button>');
		} else {
			$('.message-block.user_' + data.unique_id + ' .btnUnBan').remove();
			$('.message-block.user_' + data.unique_id + ' .delete').append('<button type="button" class="btn btn-light btnBan" data-name="' + data.username + '" data-id="' + data.unique_id + '"><svg class="icon"><use xlink:href="/img/symbols.svg#icon-ban"></use></svg><span>BANIR</span></button>');
		}
	})
});
