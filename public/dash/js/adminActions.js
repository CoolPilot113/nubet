$(document).ready(function() {
	...
	socket.on('chat', function (data) {
		...
		chat.find('.chat-scroll').append(
				'<div class="kt-widget3__item chat_mes" id="chatm_' + msg.time2 + '"><div class="kt-widget3__header"><div class="kt-widget3__user-img"><img class="kt-widget3__img" src="' + msg.avatar + '" alt=""></div><div class="kt-widget3__info"><span class="kt-widget3__username">' + msg.username + '</span><br><span class="kt-widget3__time">'+ msg.time +'</span></div><span class="kt-widget3__status"><button class="btn btn-sm btn-label-brand btn-bold" onclick="chatdelet('+ msg.time2 +')">APAGAR</button></span></div><div class="kt-widget3__body"><p class="kt-widget3__text">' + messages + '</p></div></div>');
		if(container) container.scrollTop = 99999;
	...
	socket.on('jackpot', function(r) {
		...
		if(r.type == 'update') {
			...
			for(var i = 0; i < r.data.data.chances.length; i++) {
				chances += '<div class="kt-widget4__item"><div class="kt-widget4__pic"><img src="' + r.data.data.chances[i].user.avatar + '" alt=""></div><div class="kt-widget4__info"><span class="kt-widget4__username">' + r.data.data.chances[i].user.username + '</span><p class="kt-widget4__text">' + r.data.data.chances[i].sum + 'р./' + r.data.data.chances[i].chance + '%</p></div><button class="btn btn-sm btn-outline-success btn-bold" onclick="gotRoulette('+ r.data.data.chances[i].game_id +', ' + r.data.data.chances[i].user.id + ')">Setar Vencedor</button></div>';
			}
    ...
    $('#new .minCheck').click(function() {
        if($(this).prop('checked') == true){
            $('#new .minCheck').parent().parent().parent().parent().find('.checkedDep').slideDown();
        } else {
            $('#new .minCheck').parent().parent().parent().parent().find('.checkedDep').slideUp();
        }
    });
});
