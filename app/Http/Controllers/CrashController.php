...
class CrashController extends Controller
{
    ...
    public function newBet(Request $r)
    {
        ...
        if (\Cache::has('action.user.' . $this->user->id))
            return response()->json(['msg' => 'Aguarde a ação anterior!', 'type' => 'error']);
    ...
    {
        // codigo original do matheus - não mexi
        ...
        //if ($this->settings->profit_money <= 0 && $betsPrice > 0) return round(mt_rand(1, 4).'.' . mt_rand(0, 9) . mt_rand(0, 9), 2);
        if ($this->settings->profit_money <= 0 && $betsPrice > 0) return round('1.' . mt_rand(0, 9) . mt_rand(1, 9), 2);
        //if ($this->settings->profit_money <= 0 && $betsPrice < 50) return round(mt_rand(1, 3).'.' . mt_rand(0, 9) . mt_rand(0, 9), 2);
        //if ($this->settings->profit_money <= 0 && $betsPrice > 51) return round(mt_rand(1, 3).'.' . mt_rand(0, 9) . mt_rand(0, 9), 2);
        if (!is_null($lastZero) && ($lastZero->id >= ($this->game->id + mt_rand(2, 10)))) return round('1.' . mt_rand(0, 5) . mt_rand(0, 5), 2); // CRASHAR
        if (is_null($lastZero) || ($this->game->id - $lastZero->id) >= mt_rand(2, 5)) return round('1.' . mt_rand(0, 2) . mt_rand(0, 9), 2); // CRASHAR 1.0
        */

        // codigo original russo | ta melhor do que o de cima, constantemente abaixo de 2x, nunca sobe após jogador inserir valor.
        $profit_crash = Profit::where('game', 'crash')->where('created_at', '>=', Carbon::today())->sum('sum');
        $lastZero = Crash::where('multiplier', 1)->orderBy('id', 'desc')->first();
        $betsPrice = CrashBets::where('round_id', $this->game->id)->where('fake', 0)->sum('price');
